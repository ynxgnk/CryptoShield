//
//  CryptoDetailsAPICaller.swift
//  CryptoShield
//
//  Created by Nazar Kopeika on 05.06.2023.
//
...
final class CryptoDetailsAPICaller { /* 1 */
    static let shared = CryptoDetailsAPICaller() /* 2 */
    
    private init() {} /* 3 */
    
    struct CryptoDetailsConstants { /* 4 */
    ...
    public func getCryptoDetailsData(
        completion: @escaping (Result<DogeCoinData, Error>) -> Void
    ) { /* 41 */
        guard let url = URL(string: CryptoDetailsConstants.baseUrl + CryptoDetailsConstants.endpoint + "?slug=" + CryptoDetailsConstants.doge) else { /* 43 */
        ...
        request.setValue(CryptoDetailsConstants.apikey, forHTTPHeaderField: CryptoDetailsConstants.apikeyHeader) /* 49 */
